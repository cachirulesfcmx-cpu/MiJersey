...
export default function BrandDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    client
      .getPublicBrand(params.slug)
      .then(setBrand)
      .catch((err: unknown) => {
        if (err instanceof ApiClientError && err.status === 404) {
          setNotFound(true);
        } else {
          setError(err instanceof ApiClientError ? err.message : 'No se pudo cargar la marca.');
        }
      });
  }, [client, params.slug]);
  ...
  const loadProducts = useCallback(async () => {
    try {
      const result = await client.listPublicBrandProducts(params.slug, {
        page,
        pageSize: PAGE_SIZE,
        sortBy,
        sortDir: 'asc',
  ...
  }, [client, params.slug, page, sortBy]);

  useEffect(() => {
    if (brand) void loadProducts();
  }, [brand, loadProducts]);
