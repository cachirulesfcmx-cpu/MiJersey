...
const STATUS_BY_ERROR = new Map<new (...args: never[]) => OrderError, number>([
  [OrderNotFoundError, HttpStatus.NOT_FOUND],
  [OrderNotCancellableError, HttpStatus.CONFLICT],
]);

function deriveErrorCode(error: OrderError): string {
...
export class OrdersExceptionFilter implements ExceptionFilter {
    ...
    const status =
      STATUS_BY_ERROR.get(exception.constructor as new (...args: never[]) => OrderError) ??
