...
- **AttributesModule** (`src/modules/attributes`): atributos globales reutilizables y motor de filtros/facetas. Igual que Taxonomy, no importa `CatalogModule` — tiene su propio `ProductQueryPort`. Única excepción a la regla: `CatalogModule` sí importa `AttributesModule` para exponer `GET /products/search` desde el controlador que ya es dueño de `/products` (evita colisión de rutas con `:slug`). Detalle completo en [attributes-filters.md](attributes-filters.md).
- **InventoryModule** (`src/modules/inventory`): almacenes, existencias por variante y su historial de movimientos (inmutable). Mismo patrón que Attributes/Taxonomy — propio `VariantQueryPort` de solo lectura sobre `product_variants`, sin importar `CatalogModule`. Detalle completo en [inventory.md](inventory.md).
